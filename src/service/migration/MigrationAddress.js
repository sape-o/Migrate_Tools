...
export async function MigrateGetAddress() {
  try {
    ...
    console.error('From Frontend .js : MigrateGetAddress -> ❌ Error', err)
    throw err
  }
}
...
export async function downloadCSVFileAddress(id, token) {
  const response = await fetch(`${API_URL}/migration/download-csv-address/${id}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
