...
import {
  MigrateGetAddressGroup,
  uploadFile,
  downloadCSVFileAddressGroup,
  updateStatusDeleteAddressGroup,
} from '../../service/migration/MigrationAddressGroup'
...
onBeforeMount(async () => {
  initFilters()
  try {
    files.value = await MigrateGetAddressGroup()
  } catch (err) {
...
async function exportRowCSV(row) {
  ...
  try {
    loading.value = true
    const token = getToken()
    const blob = await downloadCSVFileAddressGroup(row.id, token)
...
async function onDetail(row) {
  await router.push(`/migration/migration-address-group-detail/${row.id}`)
}
...
async function onDelete(row) {
  try {
    const token = getToken()
    loading.value = true
    const res = await updateStatusDeleteAddressGroup(row.id, token)
    if (!res.ok) {
...
<template>
  <div class="card">
    <div class="font-semibold text-xl mb-4">Uploaded Files Address Group</div>
    <DataTable
      :value="files"
      :paginator="true"
      :rows="20"
