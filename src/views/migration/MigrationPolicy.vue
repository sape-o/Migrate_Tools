...
import {
  MigrateGetPolicy,
  uploadFile,
  downloadCSVFilePolicy,
  updateStatusDeletePolicy
} from '../../service/migration/MigrationPolicy'
...
onBeforeMount(async () => {
  initFilters()
  try {
    files.value = await MigrateGetPolicy()
  } catch (err) {
...
async function exportRowCSV(row) {
  ...
  try {
    loading.value = true
    const token = getToken()
    const blob = await downloadCSVFilePolicy(row.id, token)
...
async function onDetail(row) {
  await router.push(`/migration/migration-policy-detail/${row.id}`)
}
...
async function onDelete(row) {
  try {
    const token = getToken()
    loading.value = true
    const res = await updateStatusDeletePolicy(row.id, token)
    if (!res.ok) {
...
<template>
  <div class="card">
    <div class="font-semibold text-xl mb-4">Uploaded Files Policy</div>
    <DataTable
      :value="files"
      :paginator="true"
      :rows="20"
